...
export class MoveGuard {
  ...
  check(pos, limits, now = Date.now()) {
    ...
    // Climbing: higher than a jump (or trampoline) can take you within a few seconds.
    if (!inGrace) {
      let low = null;
      for (const s of this.samples) if (now - s.t <= RISE_WINDOW_MS && (!low || s.p[1] < low.p[1])) low = s;
      if (low && pos[1] - low.p[1] > maxRise * 1.2 + SLACK_RISE) return this._violate('rise', now, low.p);
    }
    return this._accept(pos, now);
  }
